...
import utils
from models import PlaybackHistory, PlayHistory, QueryHistory, Track
...
class Music(commands.Cog):
    ...
    async def on_track_start(self, event: TrackStartEvent):
        ...
        await self.database.insert_playback_history(
            PlaybackHistory(
                ...
                interaction_id=current.extra["interaction_id"],
                message_id=message_id,
                user_id=current.requester,
                track=Track.from_track(current),
            )
        )
    ...
    async def get_default_volume(self, guild: discord.Guild) -> int:
        ...
        if volume is None:
            await self.database.set_default_volume(guild.id, settings.DEFAULT_VOLUME)
    ...
    async def play(self, interaction: discord.Interaction, query: str):
        ...
        async with lock:
            await self.database.insert_query_history(
                QueryHistory(
                    type="play",
            ...
            await self.database.insert_play_history(
                PlayHistory.from_dict(
                    {
                        "type": "command",
                        "channel_id": interaction.channel_id,
    ...
    async def on_play_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        if isinstance(error, app_commands.BotMissingPermissions):
            permissions = ", ".join(
                [
                    f"`{await interaction.translate(_T(permission, key=f'permission.{permission}'))}`"
